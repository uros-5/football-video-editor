...
class FootballEditor(object):
	...
	def checkVremenaUSekundama(self):
		...
		return True
	def canRun(self):
		if (len(self.vremenaUSekundama) == self.highlightsCounter  and len(self.vremenaUSekundama)>0):
	...
	def orderOfVids(self):
		...
		return novaLista
	def setPrvoPoluvreme(self,prvo_pol):
		self.prvo_pol = prvo_pol
	def setDrugoPoluvreme(self,drugo_pol):
		self.drugo_pol = drugo_pol
