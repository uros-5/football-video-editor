  ...
  mutations: {
    ...
    newRow(store) {
      ...
      store.highlights.push({
        ...
        toAdd: null,
        id: id,
        editing: store.compDesc.editing,
      });
    },
    deleteRow(store, id) {
      store.highlights = store.highlights.filter((item) => {
  ...
  actions: {
    getIme() {
      axios.get("http://localhost:5000/getAll").then((response) => {
        this.commit(
          "novoIme",
          JSON.parse(response.data.allComps)[0].compDesc.title
        );
      });
    },
    getCompDesc() {
    ...
    getHighlights(store) {
      const path = `http://localhost:5000/getHighlights/${Vue.$cookies.get(
        "mcID"
      )}`;
      axios.get(path).then((res) => {
        ...
        if (highlightRows.length > 0) {
          ...
          store.state.highlights.length == 0
        ) {
          this.commit("newRow");
        }
      });
    ...
    getTesting(store, vueMethod) {
      const path = `http://localhost:5000/getTesting/${Vue.$cookies.get(
        "mcID"
      )}`;
      axios.get(path).then((res) => {
    ...
    getCanCut() {
      const path = `http://localhost:5000/getCanCut/${Vue.$cookies.get(
        "mcID"
      )}`;
      axios.get(path).then((res) => {
    ...
    setCompDesc(store, payload) {
      const path = `http://localhost:5000/update/${Vue.$cookies.get(
        "mcID"
      )}/compDesc`;
      axios.post(path, store.state.compDesc).then((res) => {
        if (res.data.msg) {
          payload.showMessage();
          return null;
        }
    ...
    setHighlights(store) {
      ...
      axios.post(path, store.state.highlights).then((res) => {
  ...
  getters: {
    ...
    highlights(state) {
      return state.highlights.filter(
        (item) => item.editing === state.compDesc.editing
      );
    },
    highlightsRow(state) {
      return (id) => {
