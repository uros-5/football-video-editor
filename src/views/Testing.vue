<template>
  <div class="columns is-centered is-multiline message-container">
    <div
      class="
        message-server message-server--centered message-server--not-visible
      "
      ref="messageServer"
    >
    ...
    <div class="column is-12 testing-container">
      ...
      <h2 class="title is-6" style="grid-area: matchValue">
        {{ this.testSrc }}
      </h2>

      <h1 class="title" style="grid-area: halfTimeTest">Halftime</h1>
      <h2 class="title is-6" style="grid-area: halfTimeValue">
        {{ this.testHalftime }}
      </h2>

      <h1 class="title" style="grid-area: highlightsTest">Highlights</h1>
      <h2 class="title is-6" style="grid-area: highlightsValue">
        {{ this.testHighlights }}
      </h2>
...
export default {
  ...
  methods: {
    ...
    updateCanRun(): void | null {
      let path = `http://localhost:5000/update/${this.$cookie.get(
        "mcID"
      )}/canCut`;
      let canCut = false;
      ...
      axios.post(path, canCut).then((res) => {
        if (res.data.msg) {
          return null;
        }
      });
    },
    showMessage(element: Element): void {
      setTimeout(function () {
