<template>
  <div class="columns is-centered is-multiline final-container">
    ...
    <div class="column is-12">
      <progress
        class="progress is-primary is-medium cut-progress"
        :value="this.cutProgress"
        max="100"
      >
        15%
      </progress>
  ...
  methods: {
    ...
    updateProgress(): void {
      let cutInterval = setInterval(() => {
        axios
          .get(
            `http://localhost:5000/getCutProgress/${this.$cookie.get("mcID")}`
          )
          .then((res) => {
    ...
    updateRenderProgress(): void {
      let cutInterval = setInterval(() => {
        axios
          .get(
            `http://localhost:5000/getRenderProgress/${this.$cookie.get(
              "mcID"
            )}`
          )
          .then((res) => {
